...
function useLocationData( postId, showVenue, refreshTrigger ) {
	...
	const { locationTerms, venueName, venueLink, isResolving } = useSelect(
		( select ) => {
			...
			const { getEntityRecord, isResolving: checkResolving } =
				select( 'core' );
			...
			if (
				locationTermIds &&
				Array.isArray( locationTermIds ) &&
				locationTermIds.length > 0
			) {
				for ( const termId of locationTermIds ) {
					...
					if (
						checkResolving( 'getEntityRecord', [
							'taxonomy',
							'gatherpress_location',
							termId,
						] )
			...
			if (
				showVenue &&
				venueTermIds &&
				Array.isArray( venueTermIds ) &&
				venueTermIds.length > 0
			) {
				const venueTermId = venueTermIds[ 0 ];
				const venueTerm = getEntityRecord(
					'taxonomy',
					'_gatherpress_venue',
					venueTermId
				);
...
function useLocationHierarchy(
	...
	useEffect( () => {
		...
		if ( ! postId ) {
			setHierarchy(
				__( 'No post ID available', 'gatherpress-location-hierarchy' )
			);
			setIsLoading( false );
			return;
		}
		...
		if ( ! locationTerms || locationTerms.length === 0 ) {
			if ( showVenue && venueName ) {
				...
				const message = isInQueryLoop
					? __(
							'Location hierarchy will appear here for matching events',
							'gatherpress-location-hierarchy'
					  )
					: __(
							'No location hierarchy available for this event',
							'gatherpress-location-hierarchy'
					  );
		...
		if ( paths.length === 0 ) {
			if ( showVenue && venueName ) {
				...
				setHierarchy(
					__(
						'No location hierarchy available for selected levels',
						'gatherpress-location-hierarchy'
					)
				);
			}
...
function buildHierarchyPaths(
	...
	const leafTerms = terms.filter(
		( term ) => ! parentIds.includes( term.id )
	);
...
function buildTermPath( term, allTerms, enableLinks ) {
	...
	while ( currentTerm && depth < maxDepth ) {
		...
		if ( currentTerm.parent ) {
			const parentTerm = allTerms.find(
				( t ) => t.id === currentTerm.parent
			);
		...
		<div className="dual-range-control">
			...
			<div className="dual-range-control__labels">
				{ visibleLabels.map( ( levelLabel, index ) => {
					...
					const levelPosition =
						( index / Math.max( 1, numLevels - 1 ) ) * 100;
			...
			<div className="dual-range-control__output">
				...
				<strong>
					{ startLevel === endLevel
						...
						: `${ visibleLabels[ startLevel - minLevel ] } - ${
								visibleLabels[ endLevel - minLevel ]
						  }` }
				</strong>
			</div>
		</div>
...
function BlockInspectorControls( {
	...
	const { startLevel, endLevel, enableLinks, showVenue, separator } =
		attributes;
		...
		<InspectorControls>
			...
			>
				...
				<TextControl
					...
					help={ __(
						'Text to display between hierarchy levels',
						'gatherpress-location-hierarchy'
					) }
				/>
				...
				<ToggleControl
					...
					help={ __(
						'Link each term to its archive page',
						'gatherpress-location-hierarchy'
					) }
				/>
				...
				<ToggleControl
					...
					help={ __(
						'Display the venue name at the end of the hierarchy',
						'gatherpress-location-hierarchy'
					) }
				/>
			</PanelBody>
...
export default function Edit( { attributes, setAttributes, context } ) {
	...
	const { postId, postType, isInQueryLoop, isValidContext } =
	...
	const { locationHierarchy, isLoading: isHierarchyLoading } =
	...
	if ( ! isValidContext ) {
		return (
			<div { ...useBlockProps() }>
				{ __(
					'This block must be used within a GatherPress event',
					'gatherpress-location-hierarchy'
				) }
			</div>
		);
